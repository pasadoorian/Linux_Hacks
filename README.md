...
| Script | Description |
|--------|-------------|
| [`update.sh`](update.sh) | System update & maintenance: cache cleaning, mirror refresh, package updates (**yay by default** — pacman repos + AUR with PKGBUILD review; `-P` pacman-only, `-m` pamac), foreign-package review + orphan removal (`-o`), rebuild checks for **library/ABI** (`-r`) and **Python-version** (`-y`) breakage, pacnew listing, firmware, and kernel management. Adds an **AUR supply-chain layer**: `-A` audit (RPC metrics + maintainer re-adoption detection) and `-S` scan (live malware IOC lists + host-persistence checks). Config-file driven, clean sectioned output. Auto-elevates to root. See [`UPDATE_README.md`](UPDATE_README.md). |
